...
export class CatPrinter {
    pause = new Uint8Array([
        0x51, 0x78, 0xa3, 0x01, 0x01, 0x00, 0x10, 0x70, 0xff,
    ]);
    resume = new Uint8Array([
        0x51, 0x78, 0xa3, 0x01, 0x01, 0x00, 0x00, 0x00, 0xff,
    ]);
    mtu = 200;
    ...
    startLattice() {
        return this.send(
            this.make(
                Command.Lattice,
                new Uint8Array([
                    0xaa, 0x55, 0x17, 0x38, 0x44, 0x5f, 0x5f, 0x5f, 0x44, 0x38,
                    0x2c,
                ])
            )
        );
    ...
    endLattice() {
        return this.send(
            this.make(
                Command.Lattice,
                new Uint8Array([
                    0xaa, 0x55, 0x17, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                    0x17,
                ])
            )
        );
    ...
    prepareCamera() {
        ...
        return this.send(
            new Uint8Array([
                0x51, 0x78, 0xbc, 0x00, 0x01, 0x02, 0x01, 0x2d, 0xff,
            ])
        );
    }
